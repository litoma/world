...
- **データストレージ**: 
  ...
  - **Cloudflare Cache API**: `GET /api/snapshot` のレスポンスを5分間キャッシュ。KV Readを最大288回/日（Cron実行時のみ）に制限し、無料枠のRead超過を防止
...
│  ・データを5分間隔でポーリング（Cronの更新頻度と同期）          │
└─────────────────────┬────────────────────────────────────────┘
                      │ HTTP (GET/POST /api/*)
┌─────────────────────▼────────────────────────────────────────┐
│  Cloudflare Workers (world)                                  │
│  【API機能】                                                 │
│    - GET  /api/snapshot   → Cacheヒット or KVからデータ返却    │
│    - GET  /api/layout     → D1からユーザー別レイアウト設定の取得 │
│    - POST /api/layout     → D1へユーザー別レイアウト設定の保存   │
│  【Cron収集機能 (5分毎)】                                    │
│    - 各データソースから最新値を取得してKVに保存・時系列クレンジング│
│    - KV保存後にCaches APIのキャッシュを無効化                  │
└───────┬─────────────────────────────────┬────────────────────┘
        │ Cache API (hit/put/delete)       │ D1 (query)
┌───────▼──────────┐  ┌──────────────┐  ┌▼──────────────────────┐
│  Cloudflare      │  │  Workers KV  │  │  Cloudflare D1        │
│  Cache API       │  │  `market:data`   │  `layout` テーブル     │
│  (5分間キャッシュ │  │  (統合データ)│  │  （ユーザー別表示設定）│
│  KV Read削減)    │  │              │  │                       │
└──────────────────┘  └──────────────┘  └───────────────────────┘
```
