export interface SymbolDef {
  id: string;           // 内部ID（英数字小文字）
  label: string;        // 表示名
  category: Category;   // カテゴリ
  source: DataSource;   // データ取得元
  yahooSymbol?: string; // Yahoo Financeシンボル
  finnhubSymbol?: string; // Finnhubシンボル
  defaultOn: boolean;   // 初期表示ON/OFF
}

export type Category = 'fx' | 'index' | 'bond' | 'commodity' | 'crypto' | 'other';
export type DataSource = 'yahoo' | 'finnhub';
